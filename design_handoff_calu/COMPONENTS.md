...
Cada `<summary>` fica dentro de um `<h3>` sem estilo próprio — o tamanho de 21–22px peso 600 é do `<summary>`, não do `<h3>` — para a navegação por cabeçalhos do leitor de tela alcançar cada pergunta. `<summary>` de 64px, sinal +/− em `--calu-accent` marcado `aria-hidden="true"`: o estado é comunicado só pela semântica nativa de abertura, não pelo sinal.
